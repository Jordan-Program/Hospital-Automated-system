...
{
    public partial class Ctscan : Form
    {
        ...
        private void Ctscan_Load(object sender, EventArgs e)
        {
            timer1.Start();
            viewall();
        ...
        {
            connection();
            query = "select * from db_its.tbl_ctscanpendingcheckup where hospitalnumber = '" + lblhospitalnumber.Text + "'";
        ...
        {
            connection();
            query = "delete from db_its.tbl_ctscanpendingcheckup where hospitalnumber = '" + lblhospitalnumber.Text + "'";
        ...
        public void viewall()
        {
            ...
            query = "select * from db_its.tbl_ctscanpendingcheckup";
            command = new MySqlCommand(query, con);
            myreader = command.ExecuteReader();
            while (myreader.Read())
            {
        ...
        private void btnsearch_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                connection();
                listView1.Items.Clear();
                query = "select * from db_its.tbl_ctscanpendingcheckup where hospitalnumber like '%" + txtsearchbox.Text.Replace("'", "''") + "%' or surname like '%" + txtsearchbox.Text.Replace("'", "''") + "%' ";
                command = new MySqlCommand(query, con);
                myreader = command.ExecuteReader();
                while (myreader.Read())
                {
        ...
        private void btnsavectscanresult_Click(object sender, EventArgs e)
        {
            ...
            {
                connection();
                query = "insert into db_its.tbl_audittrail (user,action,message,datetime) values ('" + userid + "','" + "Click a button" + "','" + "User " + userid + " Add a result in CT scan " + "','" + date2 + " " + time2 + "')";
                command = new MySqlCommand(query, con);
                myreader = command.ExecuteReader();
                con.Close();

                connection();
                query = "insert into db_its.tbl_ctscanresult (hospitalnumber, surname, firstname, middlename, age, sex, ctscanresult, physician, time, date) values ('" + lblhospitalnumber.Text + "','" + surname + "','" + firstname + "','" + middlename + "','" + lblage.Text + "','" + lblsex.Text + "','" + txtctscanresult.Text.Replace("'", "''") + "','" + lblphysician.Text + "','" + time + "','" + date + "')";
                command = new MySqlCommand(query, con);
                myreader = command.ExecuteReader();
                con.Close();
                MessageBox.Show("Patient CTScan Result has been saved. Please return within 2 - 3 days for the result");
