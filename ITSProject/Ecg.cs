...
namespace ITSProject
{
    public partial class Ecg : Form
    {
        ...
        {
            connection();
            query = "select * from db_its.tbl_ecgpendingcheckup where hospitalnumber = '" + lblhospitalnumber.Text + "'";
        ...
        public void viewall()
        {
            ...
            query = "select * from db_its.tbl_ecgpendingcheckup";
            command = new MySqlCommand(query, con);
            myreader = command.ExecuteReader();
            while (myreader.Read())
            {
        ...
        {
            connection();
            query = "delete from db_its.tbl_ecgpendingcheckup where hospitalnumber = '" + lblhospitalnumber.Text + "'";
        ...
        private void btnsearch_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                connection();
                listView1.Items.Clear();
                query = "select * from db_its.tbl_ecgpendingcheckup where hospitalnumber like '%" + txtsearchbox.Text.Replace("'", "''") + "%' or surname like '%" + txtsearchbox.Text.Replace("'", "''") + "%' ";
                command = new MySqlCommand(query, con);
                myreader = command.ExecuteReader();
                while (myreader.Read())
                {
        ...
        private void btnecgresult_Click(object sender, EventArgs e)
        {
            ...
            {
                connection();
                query = "insert into db_its.tbl_audittrail (user,action,message,datetime) values ('" + userid + "','" + "Click a button" + "','" + "User " + userid + " Add a result in ECG " + "','" + date2 + " " + time2 + "')";
                command = new MySqlCommand(query, con);
                myreader = command.ExecuteReader();
                con.Close();

                connection();
                query = "insert into db_its.tbl_ecgresult (hospitalnumber, surname, firstname, middlename, age, sex, ecgresult, physician, time, date) values ('" + lblhospitalnumber.Text + "','" + surname + "','" + firstname + "','" + middlename + "','" + lblage.Text + "','" + lblsex.Text + "','" + txtecgresult.Text.Replace("'", "''") + "','" + lblphysician.Text + "','" + time + "','" + date + "')";
                command = new MySqlCommand(query, con);
                myreader = command.ExecuteReader();
                con.Close();
                MessageBox.Show("Patient ECG Result has been saved. Please return within 2 - 3 days for the result");
